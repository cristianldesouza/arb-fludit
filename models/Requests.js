...
class Requests {
	...
	getPostData(slug, language, domain, reqQuery) {
		return new Promise(async (resolve) => {
			let cmsEndoint = `${constants.CMS_ENDPOINT}/api/posts/get/${language}/${slug}/`;

			for (let query in reqQuery) {
				if (cmsEndoint.includes('?')) {
					cmsEndoint += `&${query}=${reqQuery[query]}`;
				} else {
					cmsEndoint += `?${query}=${reqQuery[query]}`;
				}
			}
			...
			try {
				...
				const response = await fetch(cmsEndoint, {
					method: 'POST',
					headers: {
	...
	getCategoryData(slug, language, domain, page, reqQuery) {
		return new Promise(async (resolve) => {
			let cmsEndoint = `${constants.CMS_ENDPOINT}/api/categories/get/${language}/${slug}/`;
	...
	getAuthorData(slug, language, domain, page, reqQuery) {
		return new Promise(async (resolve) => {
			let cmsEndoint = `${constants.CMS_ENDPOINT}/api/authors/get/${language}/${slug}/`;
	...
	getPageData(slug, language, domain, reqQuery) {
		return new Promise(async (resolve) => {
			let cmsEndoint = `${constants.CMS_ENDPOINT}/api/pages/get/${language}/${slug}/`;
	...
	getLastPosts(language, domain, reqQuery) {
		return new Promise(async (resolve) => {
			// Construct the API URL by injecting the language parameter
			let cmsEndoint = `${constants.CMS_ENDPOINT}/api/posts/get/last-posts/${language}/`;
	...
	getHomeData(language, domain, reqQuery) {
		return new Promise(async (resolve) => {
			let cmsEndoint = `${constants.CMS_ENDPOINT}/api/home/news-get/${language}/`;
			...
			try {
				...
				console.error('Erro ao buscar post(HOME):', error.message);
				return resolve(false);
			}
		});
	}
	...
	async getAggregatorData(lang, slug, reqQuery) {
		try {
			let cmsEndoint = `${constants.CMS_ENDPOINT}/api/aggregators/get/${slug}?lang=${lang}&domain=${constants.DOMAIN}`;
	...
	async getLandersAndAggregators(lang) {
		try {
			let cmsEndoint = `${constants.CMS_ENDPOINT}/api/landers/?domain=${constants.DOMAIN}&lang=${lang}`;

			const options = { method: 'GET' };

			const response = await fetch(cmsEndoint, options);

			if (!response.ok) {
				const errorData = await response.json();
				throw new Error(errorData.message || 'Falha ao buscar a lista de urls');
			}
	...
	async getSearchResults(query, domain, language) {
		try {
			// Construct the CMS search endpoint.
			// We'll define `/api/posts/search/:language` on the CMS side.
			// Or you can use a query param. This example uses the path param for language:
			let cmsEndpoint = `${
				constants.CMS_ENDPOINT
			}/api/posts/search/${language}/?q=${encodeURIComponent(query)}`;

			// We’ll POST the domain to the CMS in the body to ensure we only search in that domain.
			...
			if (!response.ok) {
				throw new Error(`Failed search request: ${response.status}`);
			}

			const data = await response.json();

			// The CMS will return { posts: [...] }. Return that array or the entire data.
			return data.posts || [];
		} catch (error) {
